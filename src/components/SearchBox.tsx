...
const destinations = [
  "Destination", "Banten", "Solo", "Jogja", "Padang", "Aceh", "Papua", "Nusa Tenggara"
];
...
export default function SearchBox() {
  ...
  return (
    <div className="bg-white shadow-md rounded-xl p-6 max-w-6xl mx-[5vw] md:mx-auto -mt-20 z-20 relative">
      {/* Baris 1 */}
      <h2 className="text-xl text-black font-semibold mb-4">Where are you going?</h2>
        ...
        {categories.map((cat, index) => (
          <button
            ...
            className={`flex-1 px-4 py-2 text-center text-sm font-medium 
              ${activeTab === cat.name ? "text-green-700 border-b-2 border-green-500" : "text-gray-500"} 
              ${index === 0 ? "rounded-l-lg" : index === 2 ? "rounded-r-lg" : ""}
              bg-white hover:bg-gray-50 transition`}
          >
            <div>{cat.name}</div>
            <div className="text-xs">{cat.count} {cat.name === "Bundling" ? "bundles" : "places"}</div>
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        {/* Check In */}
        <div>
          <label className="text-sm text-gray-600 flex flex-col gap-4">Check In</label>
          <DatePicker
            selected={startDate}
        ...
        {/* Check Out */}
        <div>
          <label className="text-sm text-gray-600 flex flex-col gap-4">Check Out</label>
          <DatePicker
            selected={endDate}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
            onClick={() => {
                ...
                router.push(`/connect/nusaexperience/search?dest=${encodeURIComponent(selected)}`)
                }
            }}
            ...
            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-6h13m0 0l-4-4m4 4l-4 4" />
            </svg>
            Show Places
        </button>
      </div>
